...
public class RelayControllerTest {
    ...
    public void setUp() {
        ...
        ReflectionTestUtils.setField(relayController, "nextNodeUrl", "http://localhost:8084");
    ...
    public void testForwardPacketSuccess() {
        ...
        assertEquals(PacketStatus.DELIVERED_TO_GATEWAY, response.getBody().getPacketStatus());
        assertEquals(1, packet.getHopCount());
        assertTrue(packet.getRouteHistory().contains("Relay-2"));
        assertEquals("Relay-2", packet.getCurrentNode());
        assertEquals("Gateway", packet.getNextNode());
